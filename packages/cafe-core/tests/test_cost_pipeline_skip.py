...
from cafe.techniques import pipeline, stage_report
...
def _pipe():
    ...
    @pipe.technique("retrieve", "bm25", cost_usd=0.002)  # fixed non-LLM cost
...
def test_skip_level_passthrough():
    ...
    factor = pipe.factor("rerank", none="chunks")
    assert "none" in factor.levels

    @pipe.compose
    async def system(config, item, ctx):
        c = await ctx.run("retrieve", query=item)
        return await ctx.run("rerank", chunks=c)   # may be skipped
